...
import (
	...
	apprepov1alpha1 "github.wdf.sap.corp/kubernetes/hub/cmd/apprepository-controller/pkg/apis/apprepository/v1alpha1"
	clientset "github.wdf.sap.corp/kubernetes/hub/cmd/apprepository-controller/pkg/client/clientset/versioned"
	appreposcheme "github.wdf.sap.corp/kubernetes/hub/cmd/apprepository-controller/pkg/client/clientset/versioned/scheme"
	informers "github.wdf.sap.corp/kubernetes/hub/cmd/apprepository-controller/pkg/client/informers/externalversions"
	listers "github.wdf.sap.corp/kubernetes/hub/cmd/apprepository-controller/pkg/client/listers/apprepository/v1alpha1"
...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			...
			_, err = c.kubeclientset.BatchV1().Jobs(namespace).Create(context.TODO(), newCleanupJob(apprepo), metav1.CreateOptions{})
			return nil
		}
	...
	if errors.IsNotFound(err) {
		...
		cronjob, err = c.kubeclientset.BatchV1beta1().CronJobs(apprepo.Namespace).Create(context.TODO(), newCronJob(apprepo), metav1.CreateOptions{})
		if err != nil {
			return err
		}

		// Trigger a manual Job for the initial sync
		_, err = c.kubeclientset.BatchV1().Jobs(apprepo.Namespace).Create(context.TODO(), newSyncJob(apprepo), metav1.CreateOptions{})
	} else if err == nil {
		...
		cronjob, err = c.kubeclientset.BatchV1beta1().CronJobs(apprepo.Namespace).Update(context.TODO(), newCronJob(apprepo), metav1.UpdateOptions{})
		if err != nil {
			return err
		}
...
func syncJobSpec(apprepo *apprepov1alpha1.AppRepository) batchv1.JobSpec {
	...
	podTemplateSpec.Spec.Containers[0].Command = []string{"/hub-token-butler"}
...
// appropriate OwnerReferences on the resource
func newCleanupJob(apprepo *apprepov1alpha1.AppRepository) *batchv1.Job {
	return &batchv1.Job{
		ObjectMeta: metav1.ObjectMeta{
			GenerateName: deleteJobName(apprepo.GetName()) + "-",
			Namespace:    apprepo.GetNamespace(),
		},
		Spec: cleanupJobSpec(apprepo),
	}
}

// cleanupJobSpec returns a batchv1.JobSpec for running the chart-repo delete job
func cleanupJobSpec(apprepo *apprepov1alpha1.AppRepository) batchv1.JobSpec {
	return batchv1.JobSpec{
		Template: corev1.PodTemplateSpec{
			Spec: corev1.PodSpec{
				// If there's an issue, delay till the next cron
				RestartPolicy:    "Never",
				ImagePullSecrets: apprepo.Spec.SyncJobPodTemplate.Spec.ImagePullSecrets,
				Containers: []corev1.Container{
					{
						Name:    "delete",
						Image:   repoSyncImage,
						Command: []string{"/chart-repo"},
						Args:    apprepoCleanupJobArgs(apprepo.GetName()),
						Env: []corev1.EnvVar{
							{
								Name: "MONGO_PASSWORD",
